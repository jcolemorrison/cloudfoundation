...
module.exports = async function validate (env, opts) {
  ...
  log.s(`No syntax errors found across ${templateFiles.length} files for ${chk.cyan(name)} template!`)
  log.i('Beginning Cloudformation Template Validation...', 0)
  ...
  log.s('No CloudFormation Template Errors found!')
  return log.i('Reminder - the validate-template API will not catch every error.  Some can only be found by deploying the full template.', 3)
}
